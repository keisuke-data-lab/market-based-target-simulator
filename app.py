...
if is_market_crash:
    st.error("⚠️ **CRASH DETECTED:** 市場環境の急変(-15%超)を検知。下限ガード(85%)作動中。")
...
with col_sub:
    ...
    if gap_percent > 0.15:
        ...
        st.markdown(f"乖離率: +{gap_percent*100:.1f}%\n\n疲弊リスクは{burnout_ratio*100:.0f}%に達しています。")
...
st.markdown("---")
st.caption("© 2026 Strategic Target Optimization Engine")